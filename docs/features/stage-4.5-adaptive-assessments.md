...
Adaptive assessments are explicitly deferred to a later stage. Open questions have
been resolved for MVP: template versioning is append-only, assessment submission is
the sync success boundary, second-brain jobs are async/idempotent, and profile reads
follow `read_child_profile?` until a future access-control stage introduces a
